...
import type { Comparativo, DivergenciaIA, Obra, ObraCompleta, OrcamentoFornecedor } from "./types";
...
export function obterObra(obraId: string): Promise<ObraCompleta> {
...
export function gerarUrlUpload(
  ...
  return apiJson(`/obras/${obraId}/orcamentos`, {
    method: "POST",
...
export function obterOrcamento(
  ...
  return apiJson(`/obras/${obraId}/orcamentos/${orcamentoId}`);
}
...
export function resolverDivergencia(
  ...
  return apiJson(`/obras/${obraId}/divergencias/${divergenciaId}`, {
    method: "PATCH",
    body: JSON.stringify({ acao }),
  });
}

export function obterComparativo(obraId: string): Promise<Comparativo> {
  return apiJson(`/obras/${obraId}/comparativo`);
}
