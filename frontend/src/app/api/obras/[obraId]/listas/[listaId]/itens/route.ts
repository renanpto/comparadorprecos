...
import {
  criarItensListaMestra,
  ApiError,
  type ItemListaMestraInput,
  type FotoListaInput,
} from "@/lib/api-client";
...
export async function POST(
  ...
  const { itens, fotos } = (await request.json()) as {
  ...
  try {
    const resultado = await criarItensListaMestra(obraId, listaId, itens, fotos ?? []);
